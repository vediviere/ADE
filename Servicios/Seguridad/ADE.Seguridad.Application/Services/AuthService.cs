...
public class AuthService
{
    ...
    Este método representa el flujo de registro de usuarios.

    IMPORTANTE:
    Actualmente el proyecto trabaja sobre la BD ADE,el flujo puede estar deshabilitado temporalmente
    o pendiente de adaptación completa a la tabla real de personas.

    Si se habilita completamente, aquí se debería:
    1. Normalizar email
    2. Validar que no exista
    3. Crear nuevo usuario/persona
    4. Guardar contraseña de forma segura
    5. Generar token si aplica
    ----------------------------------------------------------------------------
    🐾🐾🐾🐾🐾🐾*/
    public async Task<TokenResponseDto?> RegisterAsync(RegisterDto dto)
    {
        ...
        // 🐾🐾 Construcción de la entidad de usuario antes de guardarla en la BD 🐾🐾
        var usuario = new Usuario
        {
            Email = email,
            // 🐾🐾 La contraseña se transforma a hash antes de guardarse 🐾🐾
        ...
        // 🐾🐾 Guardado del nuevo usuario a través del repositorio 🐾🐾
        var creado = await _usuarioRepo.CrearAsync(usuario);

        // 🐾🐾 Recarga del usuario para obtener información completa, incluido el rol (traer Rol con Include(u => u.Rol)) 🐾🐾
        var creadoConRol = await _usuarioRepo.GetByIdAsync(creado.Id) ?? creado;

        // 🐾🐾 Generación del token para el usuario recién creado 🐾🐾
